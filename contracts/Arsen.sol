...
contract Arsen is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1000 * 10**12 * 10**9; // 1000T
    ...
    bool public BurnLpTokensEnabled = true;
    ...
    uint256 public _maxTxAmount = 500 * 10**9 * 10**9; //500B
    uint256 private numTokensSellToAddToLiquidity = 1000 * 10**6 * 10**9; //1B
    ...
    // charity donation
    struct Campaign {
        address payable benefactor; // Person starting the campaign, who puts in some ETH to donate to an Ethereum address. 
    ...
    function createCampaign (address payable charity, uint multiplier, bytes32 descriptionPart1, bytes32 descriptionPart2, bytes32 descriptionPart3, bytes32 descriptionPart4) public payable {
        require (msg.value >= minimumPledgeAmount);
        require (multiplier > 0);
        campaign[totalCampaigns].benefactor = _msgSender();
        campaign[totalCampaigns].charity = charity;
        campaign[totalCampaigns].multiplier = multiplier;
        campaign[totalCampaigns].timeStarted = block.timestamp;
    ...
    function cancelCampaign (uint campaignID) public {
    ...
    function contributeToCampaign (uint campaignID) public payable {
        ...
        campaignIDtoDonationNumberToDonation[campaignID][campaign[campaignID].donationsReceived].timeSent = block.timestamp;
    ...
    function returnHowMuchMoreBNBNeeded (uint campaignID) public view returns (uint) {
        return (campaign[campaignID].amountPledged / campaign[campaignID].multiplier - campaign[campaignID].amountRaised);
    }
    
    function generalInfo() public view returns (uint, uint, uint) {
        return (totalCampaigns, totalDonations, totalBNBraised);
    }
    
    function lookupDonation (uint campaignID, uint donationNumber) public view returns (address, uint, uint) {
    ...
    function lookupCampaignPart1 (uint campaignID) public view returns (address, address, uint, uint, uint, bytes32, bytes32) {
    ...
    function lookupCampaignPart2 (uint campaignID) public view returns (uint, bool, bool, uint, bytes32, bytes32) {
    ...
    function lookupUserDonationHistoryByCampaignID (address user) public view returns (uint[] memory) {
        return (campaignIDsDonatedToByUser[user]);
    }
    
    function lookupAmountUserDonatedToCampaign (address user, uint campaignID) public view returns (uint) {
